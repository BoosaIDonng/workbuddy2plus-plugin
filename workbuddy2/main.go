	...
	return registration{
		...
		Metadata: pluginapi.Metadata{
			...
			ConfigFields: []pluginapi.ConfigField{
				...
				{Name: "activity_auto", Type: pluginapi.ConfigFieldTypeBoolean, Description: "Enable the daily activity-map task at 10:00 local time: reports chat activity, self-checks the streak, then claims growth rewards (default true)."},
				{Name: "travel_auto", Type: pluginapi.ConfigFieldTypeBoolean, Description: "Enable the daily cat-travel task at 09:00 and 21:00 local time: adopt a buddy, send it out, claim the arrival reward (default true)."},
...
func handleExecExecute(raw []byte) ([]byte, error) {
	...
	// wbPrepareBody = model alias rewrite + vendored 2api pipeline (stream
	// forcing, tool pairing, thinking injection, effort downgrade, fingerprint
	// sanitize, prompt_cache_key) + plugin U+200B desensitizer.
	a := wbAuth(req.StorageJSON, sa)
	body := wbPrepareBody(req.Payload, req.OriginalRequest, sa, a, upstreamModel)
	...
	completion, err := wbAggregate(reader, req.Model)
	if err != nil {
		publishUsage(req.Model, upstreamModel, authUID, started, usage.Detail{}, true, 0, err.Error())
		return nil, err
	}
...
func handleExecStream(raw []byte) ([]byte, error) {
	...
	// Model alias rewrite + vendored 2api pipeline + U+200B desensitizer
	// (see handleExecExecute for the non-stream path).
	body = wbPrepareBody(body, nil, sa, a, upstreamModel)
	...
	if req.StreamID == "" {
		...
		chunks, statusCode, errCollect := collectUpstreamStream(body, sa, a, sseFramed, collector, req.HostCallbackID)
		if errCollect != nil {
	...
	httpReq, err := http.NewRequestWithContext(ctx, http.MethodPost, wbClient.ChatEndpoint(a), bytes.NewReader(body))
	if err != nil {
		cancel()
