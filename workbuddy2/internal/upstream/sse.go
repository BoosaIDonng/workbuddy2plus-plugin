...
// ChunkNormalizer 是逐帧 SSE 规整器的导出形态，供经宿主 RPC 转发 chunk 的
// 桥接方（CPA 插件流式泵）使用。帧决策与 StreamHint.writeFrame 逐字对齐：
// error 帧原样透传（白名单会剥掉 error 字段）、tool_calls name 按 index 收敛、
// 帧间 id 续传（首个真实 id 为基准，后续缺失复用）、其余帧走 normalizeFrame
// 白名单重建。跨帧状态（toolCallSeen / firstID）为每流一份。
type ChunkNormalizer struct {
	toolCallSeen map[int]bool
	firstID      string
}

// NewChunkNormalizer 为一条上游流创建规整器。
func NewChunkNormalizer() *ChunkNormalizer {
	return &ChunkNormalizer{toolCallSeen: map[int]bool{}}
}

// Normalize 处理一帧已去前缀的 data payload。返回值：
//   - out：应发出的 payload（空串 = 跳过本帧）；
//   - valid：本帧是否计为有效转发帧（JSON 解析失败的透传帧不计，空流判定用）；
//   - done：本帧是 [DONE]，读侧应立即停止。
func (n *ChunkNormalizer) Normalize(payload string) (out string, valid bool, done bool) {
	if payload == "[DONE]" {
		return "", false, true
	}
	var obj map[string]any
	if json.Unmarshal([]byte(payload), &obj) != nil {
		// 非 JSON：原样透传不计数（对齐 StreamHint.writeFrame 的降级分支）。
		return payload, false, false
	}
	if _, hasErr := obj["error"]; hasErr {
		// error 帧透传：白名单重建会剥掉 error 字段，客户端将看不到上游 code/msg。
		return payload, true, false
	}
	stripToolCallNames(obj, n.toolCallSeen)
	if n.firstID == "" {
