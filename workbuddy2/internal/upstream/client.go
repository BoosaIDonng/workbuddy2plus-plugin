...
import (
	...
	"github.com/sliverkiss/workbuddy2plus-plugin/internal/auth"
	"github.com/sliverkiss/workbuddy2plus-plugin/internal/logfmt"
)
...
// PrepareChatBodyForRealm 导出 ChatStreamContext 的出站 body 组装序列，供经
// host HTTP 桥接传输的调用方（CPA 插件桥）复用：prepareBody 全管线（脱敏 +
// effort 降级 + thinking 注入 + prompt_cache_key）后按 realm 补 console system
// （仅 global）。与 ChatStreamContext 保持逐字同序，两条路径不得漂移。
func (c *Client) PrepareChatBodyForRealm(body []byte, a *auth.Auth, conversationID string) []byte {
	prepared := c.prepareBody(body, a.Realm(), a.UID, conversationID)
	if c.globalOn(a) {
		prepared = ensureConsoleSystem(prepared)
	}
	return prepared
...
// ChatEndpoint 返回账号 realm 的 chat completions 端点 URL（chatPaths 首个
// 候选；当前两 realm 均为单路径）。
